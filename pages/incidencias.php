					        ...
					        <li class='treeview'>
					         ...
					         <li class='active treeview'>
					          <a href='incidencias.php'>
					            <i class='ion ion-android-warning'></i> <span>Incidencias</span>
					          </a>
					        </li>";
		        }
	              ...
	              <div class="box">
	                ...
	                <div class="box-header">
	                  		...
	                  		<table>
	                  			<tr>
	                  				<td style="width: 230px;" align="right">
		                  				Fecha: <input type="date" style='width: 180px;' class="form-control pull-right" name="fecha_inicio" id="fecha_inicio" value="<?php echo date('Y-m-d', strtotime($fecha)); ?>">
	                  				</td>
	                  				<td style="width: 50px;">
	                  ...
	                  <?php
	      				...
	      				?>
	                  <h3 class="box-title">Incidencias del <?= semana_nom(date("N", strtotime($fecha)))." ".date("d", strtotime($fecha))." de ".meses_nom(date("n", strtotime($fecha)))." del ".date("Y", strtotime($fecha))?></h3>
	                  <?php
	                ...
	                <div class="box-body">
	                	<div class="table-responsive">
	                  		<table id="examplenada" class="table table-bordered table-hover">
								...
								<?php
									$json = incidencias($conexion, $fecha);
									...
									for ($i=0; $i < count($json); $i++) 
									{
										...
										echo "<tr>
												<td width='4%'>".($i+1)."</td>
												<td width='5%'>".$json[$i]['id_usuario']."</td>
												<td width='15%'>".$json[$i]['nombre']."</td>
												<td width='10%'>".$json[$i]['ciudad']."</td>
												<td width='8%'>".$json[$i]['telefono']."</td>
												<td width='8%'>".$json[$i]['imei']."</td>
												<td width='25%'><input type='text' style='width: 100%;' id='comentario".$i."' value='".$json[$i]['comentario']."'></td>
												<td width='20%'>
													<select id='opcion$i' style='width: 100%;'>
								...
								?>
												<td width='7%'><button type='button' onclick="insertar_inci(<?=$json[$i]['id_usuario']?>, <?=$i?>, '<?=$fecha?>')" class='btn btn-<?=$json[$i]['boton']?> btn-sm' id='boton<?=$i?>'>Guardar</button><div id='cargando<?=$i?>'></div></td>
								<?php
										echo "	  </tr>";
									}
...
<script type="text/javascript">
	...
	function insertar_inci(usuario, contador, fecha)
	{
		var opcion = document.getElementById('opcion'+contador).value;
		var comentario = document.getElementById('comentario'+contador).value;		
		var boton = document.getElementById('boton'+contador);
		var cargando = document.getElementById('cargando'+contador);

		//alert(fecha);			
		if(opcion != "selecionar")
		{
			document.getElementById('cargando'+contador).innerHTML = "<img id='gif"+contador+"' src='../imagen/cargando.gif'>";
			var gif = document.getElementById('gif'+contador);

			$.post('../actualizar/insertar_inci.php?usu='+usuario+"&fecha="+fecha+"&codigoinci="+opcion+"&comentario="+comentario, function(data, status){
				//alert('Data: ' + data + ' Status: ' + status);
				if(data == '1')
				{
					boton.setAttribute("class", "btn btn-success btn-sm");
					document.getElementById("boton"+contador).innerHTML = 'Ok';
					gif.setAttribute("style", "display: none;");
				}
				else if(data == '2')
				{
					boton.setAttribute("class", "btn btn-warning btn-sm");
					document.getElementById("boton"+contador).innerHTML = 'Actualizado';
					gif.setAttribute("style", "display: none;");
				}
			});
		}
